...
#   Codespace: 'dev' (application-dev.yml generated by setup.sh with Docker hostnames)
#   Local:     'direct' (application-direct.yml committed in repo)
if [ "$CODESPACES" = "true" ]; then
    SPRING_PROFILE="dev"
else
    SPRING_PROFILE="direct"
fi
echo "✅ Using Spring profile: $SPRING_PROFILE"

# Run with Spring Boot (skip tests for faster startup)
mvn spring-boot:run -Dmaven.test.skip=true -Dspring-boot.run.profiles=$SPRING_PROFILE
